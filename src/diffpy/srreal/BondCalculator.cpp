...
class BondOp {
 public:
  static bool compare(const BondCalculator::BondEntry& be0,
  ...
  static bool reverse_compare(const BondCalculator::BondEntry& be0,
                              const BondCalculator::BondEntry& be1) {
    return compare(be1, be0);
  }

  static void bmerge(BondCalculator::BondDataStorage& dstbonds,
    ...
    merge(dstbonds.rbegin() + srcbonds.size(), dstbonds.rend(),
          srcbonds.rbegin(), srcbonds.rend(), dstbonds.rbegin(),
          reverse_compare);
  }

  static BondCalculator::BondEntry entryFrom(const BaseBondGenerator& bnds) {
...
void BondCalculator::addPairContribution(const BaseBondGenerator& bnds,
                                         int summationscale) {
...
void BondCalculator::finishValue() {
  ...
  last = set_difference(mbonds.begin(), mbonds.end(), mpopbonds.begin(),
                        mpopbonds.end(), mbonds.begin(), BondOp::compare);
  mbonds.erase(last, mbonds.end());
  if (mbonds.empty())
    mbonds.swap(maddbonds);
  else
    BondOp::bmerge(mbonds, maddbonds);
...
int BondCalculator::count() const { return mbonds.size(); }
