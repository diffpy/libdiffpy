...
StructureDifference CrystalStructureAdapter::diff(
...
int CrystalStructureAdapter::countSymOps() const { return msymops.size(); }
...
void CrystalStructureAdapter::addSymOp(const R3::Matrix& R,
                                       const R3::Vector& t) {
...
CrystalStructureAdapter::AtomVector CrystalStructureAdapter::expandLatticeAtom(
  const Atom& a0) const {
...
int CrystalStructureAdapter::findEqualPosition(const AtomVector& eqsites,
                                               const Atom& a0) const {
...
bool operator==(const CrystalStructureAdapter& stru0,
  ...
  bool rv = (&stru0 == &stru1) ||
            ((pstru0 == pstru1) && (stru0.msymops == stru1.msymops));
  return rv;
}
...
CrystalStructureBondGenerator::CrystalStructureBondGenerator(
  StructureAdapterConstPtr adpt)
    : PeriodicStructureBondGenerator(adpt) {
