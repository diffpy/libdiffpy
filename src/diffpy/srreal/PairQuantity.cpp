...
PairQuantity::PairQuantity()
  ...
  // attributes
  this->registerDoubleAttribute("rmin", this, &PairQuantity::getRmin,
                                &PairQuantity::setRmin);
  this->registerDoubleAttribute("rmax", this, &PairQuantity::getRmax,
                                &PairQuantity::setRmax);
}

// Public Methods ------------------------------------------------------------

const QuantityType& PairQuantity::eval() { return this->eval(mstructure); }
...
StructureAdapterPtr& PairQuantity::getStructure() { return mstructure; }

const StructureAdapterPtr& PairQuantity::getStructure() const {
  return mstructure;
}

const QuantityType& PairQuantity::value() const { return mvalue; }
...
const double& PairQuantity::getRmin() const { return mrmin; }
...
const double& PairQuantity::getRmax() const { return mrmax; }
...
bool PairQuantity::getPairMask(int i, int j) const {
  ...
  bool rv = minvertpairmask.count(ij) ? !mdefaultpairmask : mdefaultpairmask;
  return rv;
}

void PairQuantity::setTypeMask(string smbli, string smblj, bool mask) {
  static string upcaseall;
  if (upcaseall.empty()) {
    upcaseall = ALLATOMSSTR;
    transform(upcaseall.begin(), upcaseall.end(), upcaseall.begin(), ::toupper);
  }
  ...
  pair<string, string> smblij =
    (smbli > smblj) ? make_pair(smblj, smbli) : make_pair(smbli, smblj);
  // short circuit for all-all
  if (allall == smblij) {
    this->maskAllPairs(mask);
    return;
  ...
  if (ALLATOMSSTR == smbli || ALLATOMSSTR == smblj) {
    ...
    for (tpmsk = mtypemask.begin(); tpmsk != mtypemask.end();) {
      const bool eraseitem = (tpmsk != skip) && (sk == tpmsk->first.first ||
                                                 sk == tpmsk->first.second);
      if (eraseitem) {
...
bool PairQuantity::getTypeMask(const string& smbli, const string& smblj) const {
  ...
  pair<string, string> alli = (smbli > ALLATOMSSTR)
                                ? make_pair(ALLATOMSSTR, smbli)
                                : make_pair(smbli, ALLATOMSSTR);
  pair<string, string> allj = (smblj > ALLATOMSSTR)
                                ? make_pair(ALLATOMSSTR, smblj)
                                : make_pair(smblj, ALLATOMSSTR);
...
void PairQuantity::resizeValue(size_t sz) { mvalue.resize(sz); }
...
void PairQuantity::executeParallelMerge(const string& pdata) {
  ...
  transform(mvalue.begin(), mvalue.end(), pvalue.begin(), mvalue.begin(),
            plus<double>());
}
...
bool PairQuantity::hasTypeMask() const { return !mtypemask.empty(); }
...
void PairQuantity::updateMaskData() {
  ...
  // For type masking propagate atom types to corresponding atom indices.
    ...
    // build a list of type masks with all-masks at the beginning
    ...
    for (tpmsk = mtypemask.begin(); tpmsk != mtypemask.end(); ++tpmsk) {
      ...
      if (hasall)
        ...
        orderedpairs.push_back(tpmsk->first);
    }
...
bool PairQuantity::setPairMaskValue(int i, int j, bool mask) {
  ...
  pair<int, int> ij = (i > j) ? make_pair(j, i) : make_pair(i, j);
  bool rv;
  rv = (mask == mdefaultpairmask) ? minvertpairmask.erase(ij)
...
StructureAdapterPtr replacePairQuantityStructure(PairQuantity& pq,
