...
class EmptyStructureAdapter : public StructureAdapter {
 public:
  ...
  virtual int countSites() const { return 0; }

  // reusing StructureAdapter::totalOccupancy()
  // reusing StructureAdapter::numberDensity()

  virtual const std::string& siteAtomType(int idx) const { throw outofrange; }

  virtual const R3::Vector& siteCartesianPosition(int idx) const {
    throw outofrange;
  }

  virtual int siteMultiplicity(int idx) const { throw outofrange; }

  virtual double siteOccupancy(int idx) const { throw outofrange; }

  virtual bool siteAnisotropy(int idx) const { throw outofrange; }
