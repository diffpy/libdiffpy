...
void LatticeParameters::update() {
  ...
  const double Vunit =
    sqrt(1.0 + 2.0 * ca * cb * cg - ca * ca - cb * cb - cg * cg);
  ar = sa / (a * Vunit);
  br = sb / (b * Vunit);
  cr = sg / (c * Vunit);
  car = (cb * cg - ca) / (sb * sg);
  sar = sqrt(1.0 - car * car);
  cbr = (ca * cg - cb) / (sa * sg);
  sbr = sqrt(1.0 - cbr * cbr);
  cgr = (ca * cb - cg) / (sa * sb);
  sgr = sqrt(1.0 - cgr * cgr);
...
PointsInSphere::PointsInSphere(double rmin, double rmax, double _a, double _b,
                               double _c, double _alpha, double _beta,
                               double _gamma)
    : _Rmin(rmin),
      _Rmax(rmax),
...
void PointsInSphere::next() { next_o(); }

bool PointsInSphere::finished() const { return !(m() < this->hi_m); }

// data access

const double& PointsInSphere::Rmin() const { return this->_Rmin; }

const double& PointsInSphere::Rmax() const { return this->_Rmax; }

const int* PointsInSphere::mno() const { return this->_mno; }

int PointsInSphere::m() const { return this->_mno[0]; }

int PointsInSphere::n() const { return this->_mno[1]; }

int PointsInSphere::o() const { return this->_mno[2]; }
...
void ReflectionsInQminQmax::rewind() { this->sph.rewind(); }

void ReflectionsInQminQmax::next() { this->sph.next(); }

bool ReflectionsInQminQmax::finished() const { return this->sph.finished(); }

// data access

const double& ReflectionsInQminQmax::Qmin() const { return this->_Qmin; }

const double& ReflectionsInQminQmax::Qmax() const { return this->_Qmax; }

const int* ReflectionsInQminQmax::hkl() const { return this->sph.mno(); }

int ReflectionsInQminQmax::h() const { return this->sph.m(); }

int ReflectionsInQminQmax::k() const { return this->sph.n(); }

int ReflectionsInQminQmax::l() const { return this->sph.o(); }

double ReflectionsInQminQmax::Q() const { return 2.0 * M_PI * sph.r(); }

double ReflectionsInQminQmax::d() const { return 1.0 / sph.r(); }
...
ReflectionsInDmaxDmin::ReflectionsInDmaxDmin(double dmax, double dmin,
                                             ...
                                             double _alpha, double _beta,
                                             double _gamma)
    : ReflectionsInQminQmax(2.0 * M_PI / dmax, 2.0 * M_PI / dmin, _a, _b, _c,
                            _alpha, _beta, _gamma),
      _Dmin(dmin),
      _Dmax(dmax) {}
...
const double& ReflectionsInDmaxDmin::Dmin() const { return this->_Dmin; }

const double& ReflectionsInDmaxDmin::Dmax() const { return this->_Dmax; }
