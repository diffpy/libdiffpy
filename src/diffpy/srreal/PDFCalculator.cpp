...
bool _initialQstepUpdate(const PDFC* pc) {
  ...
  const bool rv = (pc->getStructure() == emptyStructureAdapter()) &&
...
PDFCalculator::PDFCalculator()
  ...
  // attributes
  this->registerDoubleAttribute("qmin", this, &PDFCalculator::getQmin,
                                &PDFCalculator::setQmin);
  this->registerDoubleAttribute("qmax", this, &PDFCalculator::getQmax,
                                &PDFCalculator::setQmax);
  this->registerDoubleAttribute("qstep", this, &PDFCalculator::getQstep);
  this->registerDoubleAttribute("rmin", this, &PDFCalculator::getRmin,
                                &PDFCalculator::setRmin);
  this->registerDoubleAttribute("rmax", this, &PDFCalculator::getRmax,
                                &PDFCalculator::setRmax);
  this->registerDoubleAttribute("rstep", this, &PDFCalculator::getRstep,
                                &PDFCalculator::setRstep);
  this->registerDoubleAttribute("maxextension", this,
                                &PDFCalculator::getMaxExtension,
                                &PDFCalculator::setMaxExtension);
...
QuantityType PDFCalculator::getExtendedPDF() const {
  ...
  const bool skipfft = !eps_lt(this->getQmax(), M_PI / this->getRstep()) &&
...
QuantityType PDFCalculator::getExtendedRDF() const {
  ...
  double rdf_scale =
    (totocc * sfavg == 0.0) ? 0.0 : 1.0 / (totocc * sfavg * sfavg);
  QuantityType::iterator iirdf = rdf.begin();
  QuantityType::const_iterator iival, iival_last;
  iival =
    this->value().begin() + this->extendedRminSteps() - this->rcalcloSteps();
  iival_last =
    this->value().begin() + this->extendedRmaxSteps() - this->rcalcloSteps();
...
QuantityType PDFCalculator::getExtendedF() const {
  ...
  assert(rv.empty() ||
         eps_eq(M_PI, this->getQstep() * rv.size() * this->getRstep()));
  return rv;
}
...
QuantityType PDFCalculator::getQgrid() const { return pdfutils_getQgrid(this); }
...
const double& PDFCalculator::getQmin() const { return mqmin; }
...
QuantityType PDFCalculator::getRgrid() const { return pdfutils_getRgrid(this); }
...
const double& PDFCalculator::getRstep() const { return mrstep; }
...
const double& PDFCalculator::getMaxExtension() const { return mmaxextension; }
...
QuantityType PDFCalculator::applyBaseline(const QuantityType& x,
...
void PDFCalculator::resetValue() {
  ...
  if (this->getBaseline()->type() == "linear") {
    double partialpdfscale =
      (0.0 == mstructure_cache.totaloccupancy)
        ? 0.0
...
void PDFCalculator::restorePartialValue() {
  ...
  if (leftshift >= 0)
    ...
    ti += min(-leftshift, int(mvalue.size()));
...
double PDFCalculator::extFromPeakTails() const {
  ...
  double maxfwhm = pwm.maxWidth(mstructure, this->getRmin(), this->getRmax());
  const PeakProfile& pkf = *(this->getPeakProfile());
...
int PDFCalculator::rcalcloSteps() const { return mrlimits_cache.rcalclosteps; }

int PDFCalculator::rcalchiSteps() const { return mrlimits_cache.rcalchisteps; }
...
double PDFCalculator::sfAverage() const { return mstructure_cache.sfaverage; }
...
void PDFCalculator::cacheStructureData() {
  ...
  for (auto&& ij : minvertpairmask) {
    ...
    double occij = sumscale * mstructure->siteOccupancy(i) *
                   mstructure->siteMultiplicity(i) *
                   mstructure->siteOccupancy(j) *
                   mstructure->siteMultiplicity(j);
    invmasktotal += occij;
  }
  if (totocc > 0.0) invmasktotal /= totocc;
  mstructure_cache.activeoccupancy =
    (mdefaultpairmask) ? (totocc - invmasktotal) : invmasktotal;
}
...
void PDFCalculator::cacheRlimitsData() {
  ...
  const double& dr = this->getRstep();
  mrlimits_cache.extendedrminsteps =
    max(0, pdfutils_rminSteps(rmin - ext_ripples, dr));
  mrlimits_cache.extendedrmaxsteps = pdfutils_rmaxSteps(rmax + ext_ripples, dr);
  mrlimits_cache.rcalclosteps =
    max(0, pdfutils_rminSteps(rmin - ext_total, dr));
