...
template <class T>
std::vector<std::string> siteIndicesToTypes(const StructureAdapterPtr& stru,
                                            const T& indices) {
...
inline StructureAdapterPtr createStructureAdapter(
  const StructureAdapter& stru) {
  return stru.clone();
}
...
template <class T>
StructureAdapterPtr convertToStructureAdapter(
  const boost::shared_ptr<T>& stru) {
...
inline StructureAdapterPtr convertToStructureAdapter(
  StructureAdapterConstPtr stru) {
  StructureAdapterPtr rv =
