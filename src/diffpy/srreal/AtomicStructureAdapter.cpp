...
bool operator==(const Atom& a0, const Atom& a1) {
  bool rv = (&a0 == &a1) ||
            (a0.atomtype == a1.atomtype && (a0.xyz_cartn == a1.xyz_cartn) &&
             a0.occupancy == a1.occupancy && a0.anisotropy == a1.anisotropy &&
             a0.uij_cartn == a1.uij_cartn);
  return rv;
}

bool operator!=(const Atom& a0, const Atom& a1) { return !(a0 == a1); }
...
int AtomicStructureAdapter::countSites() const { return matoms.size(); }
...
StructureDifference AtomicStructureAdapter::diff(
  ...
  for (ii = symdiffatoms.begin(); ii != symdiffatoms.end(); ++ii) {
    if (ii->second >= 0)
      sd.pop0.push_back(ii->second);
    else
      sd.add1.push_back(-1 * ii->second - 1);
  }
...
void AtomicStructureAdapter::clear() { matoms.clear(); }
