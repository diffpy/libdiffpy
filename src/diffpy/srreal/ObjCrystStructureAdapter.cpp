...
CrystalStructureAdapter::SymOpVector fetchSymmetryOperations(
  const ObjCryst::SpaceGroup& spacegroup) {
...
StructureAdapterPtr createStructureAdapter(const ObjCryst::Crystal& cryst) {
  const double radtodeg = 180 / M_PI;
  CrystalStructureAdapterPtr adpt(new CrystalStructureAdapter);
  adpt->setLatPar(cryst.GetLatticePar(0), cryst.GetLatticePar(1),
                  cryst.GetLatticePar(2), radtodeg * cryst.GetLatticePar(3),
...
StructureAdapterPtr createStructureAdapter(const ObjCryst::Molecule& molecule) {
