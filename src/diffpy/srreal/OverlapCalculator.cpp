...
double OverlapCalculator::totalSquareOverlap() const {
  ...
  for (int i = 0; i < cntsites; ++i) {
    rv += sqoverlap[i] * mstructure->siteMultiplicity(i) *
...
double OverlapCalculator::flipDiffTotal(int i, int j) const {
  ...
  bool sameradii = (i == j) || (mstructure_cache.siteradii[i] ==
  ...
  for (idx = allids.begin(); idx != allids.end(); ++idx) {
    ...
    double sqscale = ((i1 == j1) ? 1 : 2) * mstructure->siteOccupancy(i1) *
                     mstructure->siteOccupancy(j1) *
...
unordered_map<string, double> OverlapCalculator::coordinationByTypes(
  int i) const {
...
void OverlapCalculator::addPairContribution(const BaseBondGenerator& bnds,
                                            int summationscale) {
...
QuantityType OverlapCalculator::subvector(
  int offset, OverlapCalculator::OverlapFlag flag) const {
...
const std::list<int>& OverlapCalculator::getNeighborIds(int k) const {
  ...
  const NbList& rv = (nbit == mneighborids.end()) ? noneighbors : nbit->second;
  return rv;
}
