...
void BVParametersTable::resetAtomValences() { matomvalence.clear(); }
...
const BVParam& BVParametersTable::lookup(const string& smbl0,
                                         const string& smbl1) const {
  const int v0 = this->getAtomValence(smbl0);
  const int v1 = this->getAtomValence(smbl1);
  const BVParam& rv =
    this->lookup(atomBareSymbol(smbl0), v0, atomBareSymbol(smbl1), v1);
  return rv;
}
...
void BVParametersTable::setCustom(const string& atom0, int valence0,
                                  const string& atom1, int valence1, double Ro,
                                  double b, std::string ref_id) {
...
const BVParametersTable::SetOfBVParam& BVParametersTable::getAllCustom() const {
  return mcustomtable;
}

BVParametersTable::SetOfBVParam BVParametersTable::getAll() const {
