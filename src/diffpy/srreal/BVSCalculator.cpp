...
BVSCalculator::BVSCalculator() {
  ...
  this->registerDoubleAttribute("rmaxused", this, &BVSCalculator::getRmaxUsed);
}
...
double BVSCalculator::getValencePrecision() const { return mvalenceprecision; }

double BVSCalculator::getRmaxUsed() const {
  double rv =
    min(this->getRmax(), this->rmaxFromPrecision(this->getValencePrecision()));
  return rv;
}
...
double BVSCalculator::rmaxFromPrecision(double eps) const {
  ...
  typedef std::unordered_set<pair<string, int>,
  ...
  for (int i = 0; i < this->countSites(); ++i) {
    allsymvals.insert(
      make_pair(mstructure_cache.baresymbols[i], mstructure_cache.valences[i]));
  }
  ...
  for (sv0 = allsymvals.begin(); sv0 != allsymvals.end(); ++sv0) {
    for (sv1 = sv0; sv1 != allsymvals.end(); ++sv1) {
      const BVParam& bp =
        bvtb.lookup(sv0->first, sv0->second, sv1->first, sv1->second);
      bpused.insert(bp);
    }
  }
  double rv = 0.0;
