...
bool BaseBondGenerator::finished() const { return msite_current >= msite_last; }
...
void BaseBondGenerator::selectSites(SiteIndices::const_iterator first,
...
const double& BaseBondGenerator::getRmin() const { return mrmin; }

const double& BaseBondGenerator::getRmax() const { return mrmax; }

int BaseBondGenerator::site0() const { return msite_anchor; }

int BaseBondGenerator::site1() const { return *msite_current; }

int BaseBondGenerator::multiplicity() const {
  return mstructure->siteMultiplicity(this->site0());
}

const R3::Vector& BaseBondGenerator::r0() const { return mr0; }

const R3::Vector& BaseBondGenerator::r1() const { return mr1; }

const double& BaseBondGenerator::distance() const { return mdistance; }

const R3::Vector& BaseBondGenerator::r01() const { return mr01; }
...
double BaseBondGenerator::msd() const {
  static R3::Vector s;
  s = this->r01();
  double msd0 = meanSquareDisplacement(
    this->Ucartesian0(), s, mstructure->siteAnisotropy(this->site0()));
  double msd1 = meanSquareDisplacement(
    this->Ucartesian1(), s, mstructure->siteAnisotropy(this->site1()));
  double rv = msd0 + msd1;
  return rv;
}
...
bool BaseBondGenerator::iterateSymmetry() { return false; }
...
void BaseBondGenerator::advanceWhileInvalid() {
  while (!this->finished() && (this->bondOutOfRange() || this->atSelfPair())) {
    this->getNextBond();
  }
}
...
void BaseBondGenerator::setFinishedFlag() { msite_current = msite_last; }
