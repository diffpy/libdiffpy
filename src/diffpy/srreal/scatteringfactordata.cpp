...
class WaasKirfFormula {
 public:
  ...
  double electronatstol(double stol) const {
    ...
    double rv = 0.023934 * (this->Z() - this->xrayatstol(stol)) / (stol * stol);
    return rv;
  }
...
class wksmbl_equal
 ...
 public:
  bool operator()(const WaasKirfFormula& wk0,
                  const WaasKirfFormula& wk1) const {
...
typedef unordered_set<WaasKirfFormula, wksmbl_hash, wksmbl_equal>
  SetOfWKFormulas;
...
const SetOfWKFormulas& getWKFormulasSet() {
  ...
  for (WaasKirfFormula wk; fp >> line;) {
    if (line.iscomment() && (0 == line.words[0].compare(0, 2, "#S"))) {
      if (line.wcount() < 3) {
        throw line.format_error(wkfile, "Expected at least 3 columns of data.");
      }
    ...
    if (line.iscomment() && (0 == line.words[0].compare(0, 2, "#L"))) {
      if (wk.symbol.empty()) {
        throw line.format_error(
          wkfile, "Missing \"#S\" line with atom symbol definition.");
      }
      fp >> line;
      if (line.wcount() != 11) {
      ...
      fpline >> wk.a[0] >> wk.a[1] >> wk.a[2] >> wk.a[3] >> wk.a[4] >> wk.c >>
        wk.b[0] >> wk.b[1] >> wk.b[2] >> wk.b[3] >> wk.b[4];
      if (!fpline) {
        throw line.format_error(
          wkfile, "Line should contain 11 floating point values.");
      }
      if (the_set->count(wk)) {
...
ElectronNumberStorage& getElectronNumberTable() {
  ...
  if (!entable) {
    ...
    if (entable->size() < mintablesize) {
      ostringstream emsg;
      emsg << "Incomplete file.  Expected " << mintablesize << " items loaded "
           << entable->size() << ".";
...
const NeutronBCStorage& getNeutronBCTable() {
  ...
  while (fp >> line) {
    if (line.isignored()) continue;
    if (line.wcount() != 11) {
      throw line.format_error(nsffile, "Expected 11 comma-separated items.");
    }
    ...
    if (p2 != string::npos) {
      ...
      bool addel =
        (chlf == "100") || (!bctable->count(el) && *chlf.rbegin() == 'Y');
      if (addel) {
        if (bctable->count(el)) {
...
double bcneutron(const string& smbl) {
  ...
  string smblnocharge = (pe != string::npos) ? smbl.substr(0, pe + 1) : smbl;
