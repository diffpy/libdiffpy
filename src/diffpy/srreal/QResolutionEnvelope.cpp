...
QResolutionEnvelope::QResolutionEnvelope() {
  this->setQdamp(0.0);
  this->registerDoubleAttribute("qdamp", this, &QResolutionEnvelope::getQdamp,
                                &QResolutionEnvelope::setQdamp);
}
...
double QResolutionEnvelope::operator()(const double& r) const {
  double rv = (mqdamp > 0.0) ? exp(-pow(r * mqdamp, 2) / 2) : 1.0;
  return rv;
}

void QResolutionEnvelope::setQdamp(double sc) { mqdamp = sc; }

const double& QResolutionEnvelope::getQdamp() const { return mqdamp; }
