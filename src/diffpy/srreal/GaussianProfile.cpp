...
void GaussianProfile::setPrecision(double eps) {
  ...
  if (eps1 < 1.0)
    mhalfboundrel = sqrt(-log(eps1) / (4 * M_LN2));
  else
    mhalfboundrel = 0.0;
}
...
DIFFPY_INSTANTIATE_SERIALIZATION(diffpy::srreal::GaussianProfile)
BOOST_CLASS_EXPORT_IMPLEMENT(diffpy::srreal::GaussianProfile)
