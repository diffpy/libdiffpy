...
Attributes::CountDoubleAttrVisitor::CountDoubleAttrVisitor(const string& name)
    : mname(name), mcount(0) {}

void Attributes::CountDoubleAttrVisitor::visit(const Attributes& a) {
  mcount += a.mdoubleattrs.count(mname);
}

int Attributes::CountDoubleAttrVisitor::count() const { return mcount; }

// GetDoubleAttrVisitor

Attributes::GetDoubleAttrVisitor::GetDoubleAttrVisitor(const string& name)
    : mname(name) {}

void Attributes::GetDoubleAttrVisitor::visit(const Attributes& a) {
...
double Attributes::GetDoubleAttrVisitor::getValue() const { return mvalue; }

// SetDoubleAttrVisitor

Attributes::SetDoubleAttrVisitor::SetDoubleAttrVisitor(const string& name,
                                                       double value)
    : mname(name), mvalue(value) {}

void Attributes::SetDoubleAttrVisitor::visit(const Attributes& a) {
  const char* emsg = "Cannot set attribute of a const instance.";
  throw logic_error(emsg);
}

void Attributes::SetDoubleAttrVisitor::visit(Attributes& a) {
...
Attributes::NamesOfDoubleAttributesVisitor::NamesOfDoubleAttributesVisitor(
  bool excludereadonly) {
  mexcludereadonly = excludereadonly;
}

void Attributes::NamesOfDoubleAttributesVisitor::visit(const Attributes& a) {
...
const set<string>& Attributes::NamesOfDoubleAttributesVisitor::names() const {
  return mnames;
}

// Non-member Helpers --------------------------------------------------------

void registerBaseDoubleAttribute(Attributes* obj, const string& name,
                                 BaseDoubleAttribute* pa) {
  obj->mdoubleattrs[name].reset(pa);
}

void throwDoubleAttributeReadOnly() {
  const char* emsg = "Cannot change value of read-only DoubleAttribute.";
