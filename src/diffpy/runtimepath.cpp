...
#else
  "../share/diffpy/libdiffpy-" STRINGIFY(DIFFPY_VERSION_MAJOR) "." STRINGIFY(
...
LineReader::LineReader() : lineno(0), commentmark("#") {}
...
bool LineReader::isblank() const { return words.empty(); }

size_t LineReader::wcount() const { return words.size(); }

runtime_error LineReader::format_error(const string& filename,
                                       const string& edetail) {
  ostringstream emsg;
  emsg << "Invalid data format in " << filename << " line " << this->lineno
       << '.';
