...
class TestLattice : public CxxTest::TestSuite {
 ...
 public:
  ...
  void test_setLatPar() {
    ...
    R3::Matrix base_check(sqrt(0.75), -0.5, 0.0, 0.0, +2.0, 0.0, 0.0, +0.0,
                          3.0);
    TS_ASSERT(allclose(base_check, lattice->base()));
    R3::Matrix recbase_check(sqrt(4.0 / 3), sqrt(1.0 / 12), 0.0, 0.0, 0.5, 0.0,
                             0.0, 0.0, 1.0 / 3.0);
  ...
  void test_setLatBase() {
    ...
    R3::Matrix base_check(va[0], va[1], va[2], vb[0], vb[1], vb[2], vc[0],
                          vc[1], vc[2]);
    TS_ASSERT(allclose(base_check, lattice->base()));
    R3::Matrix recbase_check(0.5, -0.5, 0.5, 0.5, 0.5, -0.5, -0.5, 0.5, 0.5);
    TS_ASSERT(allclose(recbase_check, lattice->recbase()));
    lattice->setLatPar(lattice->a(), lattice->b(), lattice->c(), 44.0, 66.0,
                       88.0);
    TS_ASSERT(!allclose(base_check, lattice->base()));
    TS_ASSERT(!allclose(recbase_check, lattice->recbase()));
    lattice->setLatPar(lattice->a(), lattice->b(), lattice->c(), 60.0, 60.0,
                       60.0);
    TS_ASSERT(allclose(base_check, lattice->base()));
    TS_ASSERT(allclose(recbase_check, lattice->recbase()));
  }
  ...
  void test_dist() {
    ...
    TS_ASSERT_DELTA(3.0, lattice->distance(va, vb), precision);
    lattice->setLatPar(2.0, 2.0, 2.0, 90.0, 90.0, 90.0);
    TS_ASSERT_DELTA(6.0, lattice->distance(va, vb), precision);
  }

  void test_angle() {
    R3::Vector va(1.0, 0.0, 0.0);
    R3::Vector vb(0.0, 1.0, 0.0);
    TS_ASSERT_DELTA(90.0, lattice->angledeg(va, vb), precision);
    lattice->setLatPar(2.0, 2.0, 2.0, 90.0, 90.0, 120.0);
    TS_ASSERT_DELTA(120.0, lattice->angledeg(va, vb), precision);
  }
