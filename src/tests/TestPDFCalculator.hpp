...
class TestPDFCalculator : public CxxTest::TestSuite {
 ...
 public:
  ...
  void test_access_PeakProfile() {
    ...
    TS_ASSERT_EQUALS(0.2, mpdfc->getDoubleAttr("peakprecision"));
    TS_ASSERT_THROWS(mpdfc->setPeakProfileByType("invalid"), logic_error);
  }
  ...
  void test_getQgrid() { TS_ASSERT_EQUALS(1024u, mpdfc->getQgrid().size()); }

  void test_getQmin() { TS_ASSERT_EQUALS(0.0, mpdfc->getQmin()); }

  void test_getQmax() { TS_ASSERT_DELTA(100 * M_PI, mpdfc->getQmax(), meps); }
  ...
  void test_serialization() {
    ...
    TS_ASSERT_EQUALS(string("constant"), pdfc1->getPeakWidthModel()->type());
    ...
    TS_ASSERT_EQUALS(1.1, pdfc1->getScatteringFactorTable()->lookup("H"));
  }
