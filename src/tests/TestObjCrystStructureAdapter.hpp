...
double testmsd0(const Tstru& stru, const Tbnds& bnds) {
  ...
  double rv =
    meanSquareDisplacement(bnds->Ucartesian0(), bnds->r01(), anisotropy0);
  return rv;
}
...
double testmsd1(const Tstru& stru, const Tbnds& bnds) {
  ...
  double rv =
    meanSquareDisplacement(bnds->Ucartesian1(), bnds->r01(), anisotropy1);
  return rv;
}
...
class TestObjCrystStructureBondGenerator : public CxxTest::TestSuite {
 ...
 public:
  ...
  void test_typeid() {
    ...
    TS_ASSERT(typeid(CrystalStructureBondGenerator) == typeid(r_nibnds));
  }
  ...
  void test_bondCountNickel() {
    ...
    // which sums to 18 neighbors within radius 3.6
    m_nibnds->setRmin(0);
  ...
  void test_LiTaO3() {
    ...
    for (bnds->rewind(); !bnds->finished(); bnds->next()) {
      ...
      // other 3 tantalums are related by triple axis and
...
class TestObjCrystMoleculeAdapter : public CxxTest::TestSuite {
 ...
 public:
  ...
  void test_countSites() { TS_ASSERT_EQUALS(60, m_c60->countSites()); }

  void test_totalOccupancy() { TS_ASSERT_EQUALS(60, m_c60->totalOccupancy()); }
