...
preferable to compile in the `develop` build mode, which includes
debugging information and allows C-assertions checks:

    scons -j4 build=develop

The build process checks for a presence of sconsvars.py file, which
can be used to preset the SCons variables.  The SCons construction
environment can be further customized in a sconscript.local script.
